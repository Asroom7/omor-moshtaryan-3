...
/* تشخیص مشتری تکراری: اول شماره تماس، سپس نام و نام خانوادگی؛ در صورت نبود تطابق، مشتری جدید ساخته می‌شود */
async function matchOrCreateCustomer(phone, firstName, lastName, address) {
  ...
  let customer = null;
  if (normPhone) {
    customer = state.customers.find((c) => normalizePhoneForMatch(c.phone) === normPhone);
  }
  if (!customer && normName) {
    customer = state.customers.find((c) => normalizeNameForMatch(customerFullName(c)) === normName);
  }
  if (customer) {
    const patch = {};
    ...
    if (Object.keys(patch).length) await idb.put('customers', { ...customer, ...patch, id: customer.id });
    return customer.id;
  }
  return idb.add('customers', {
    firstName, lastName, phone: phone || '', address: address || '',
    lastContact: null, nextFollowUp: null, status: 'جدید', description: '', createdAt: todayISO(),
  });
}
  ...
  return `
    <div class="rec-card" style="border-right-color:${color};" data-id="${p.id}">
      <div class="rec-card__body">
        ...
        <div class="rec-card__id"${isPastPending ? ' style="color:var(--accent);font-weight:700;"' : ''}>${fmtId('P', p.id)} · ${formatJalaliDisplay(p.date)}</div>
        <div class="rec-card__meta">
...
/* --- ثبت دستیِ واریزی جدید؛ تاریخ آینده = پیگیری (فعلاً بدون اتصال پیامک) --- */
function openPaymentAddForm() {
  ...
  const html = `
    ...
    <form id="paymentAddForm">
      ...
      <div id="futureFuFields" style="display:none;">
        <div class="field__hint" style="margin:0 2px 10px;color:var(--accent);">تاریخ انتخاب‌شده در آینده است — این مورد به «پیگیری‌ها» اضافه می‌شود.</div>
        <div class="field-row">
        ...
        <div class="field"><label>زمان پیگیری</label><input name="fuTime" type="time" value="09:00"></div>
      </div>
      <div class="modal__actions">
        <button type="button" class="btn secondary block" data-close-modal>انصراف</button>
  ...
  function syncFutureVisibility() {
    const isFuture = !!(formData.date && formData.date > todayISO());
  ...
  wrap.querySelector('#paymentAddForm').addEventListener('submit', async (e) => {
    ...
    if (isFuture) {
      ...
      if (!fName || !lName) { toast('برای پیگیری آینده، نام و نام خانوادگی الزامی است'); return; }
      const phone = fd.get('fuPhone').trim();
      const time = fd.get('fuTime') || '09:00';
      await idb.add('payments', {
        amount, date: formData.date,
        customerId: null, customerName: `${fName} ${lName}`.trim(), phone, address: '', description: '',
        status: 'followup', source: 'manual', bank: 'meli', followUpAt: `${formData.date}T${time}:00`,
...
/* --- پیگیری‌ها: واریزی‌های ثبت‌شده با تاریخ آینده --- */
function formatFollowUpTime(followUpAt) {
  if (!followUpAt || followUpAt.length < 16) return '';
  const hm = followUpAt.slice(11, 16);
  return `ساعت ${faDigits(hm)}`;
}
function followUpDotState(p) {
  const today = todayISO();
  const dateArrived = p.date <= today;
  const timeArrived = !!(p.followUpAt && new Date(p.followUpAt) <= new Date());
  if (timeArrived) return 'blink';
  if (dateArrived) return 'static';
  return 'none';
}
  ...
  return `
    <div class="rec-card" style="border-right-color:var(--accent);" data-id="${p.id}">
      <div class="rec-card__body">
        ...
        <div class="rec-card__id">${fmtId('P', p.id)} · موعد: ${formatJalaliDisplay(p.date)}${p.followUpAt ? ` · ${formatFollowUpTime(p.followUpAt)}` : ''}</div>
        <div class="rec-card__meta">
          <span>${ic('wallet')}${fmtPrice(p.amount)}</span>
          ${p.phone ? `<a class="tel-link" href="tel:${esc(p.phone)}" onclick="event.stopPropagation()">${ic('phone')}${esc(p.phone)}</a>` : ''}
        </div>
...
function computeFollowUpList() {
  return state.payments.filter((p) => p.status === 'followup')
    .sort((a, b) => (a.followUpAt || a.date).localeCompare(b.followUpAt || b.date));
}
...
function openFollowUpActionSheet(id) {
  const rec = byId(state.payments, id);
  if (!rec) return;
  const name = (rec.customerName || '').trim() || fmtId('P', rec.id);
  const html = `
    <div class="modal__handle"></div>
...
/* پیگیری مجدد: انتخاب تاریخ و زمان جدید */
function openFollowUpRescheduleForm(rec) {
  const formData = { date: rec.date && rec.date > todayISO() ? rec.date : todayISO() };
  const existingTime = rec.followUpAt ? rec.followUpAt.slice(11, 16) : '09:00';
  const html = `
    <div class="modal__handle"></div>
    <h3 class="modal__title">پیگیری مجدد</h3>
    <form id="fuRescheduleForm">
      ${dateFieldHTML('date', formData.date, 'تاریخ پیگیری بعدی', true)}
      <div class="field"><label>زمان پیگیری</label><input name="fuTime" type="time" value="${esc(existingTime)}"></div>
      <div class="modal__actions">
        <button type="button" class="btn secondary block" data-close-modal>انصراف</button>
        <button type="submit" class="btn primary block">${ic('check')}ذخیره</button>
      </div>
  ...
  wrap.querySelector('#fuRescheduleForm').addEventListener('submit', async (e) => {
    ...
    const now = Date.now();
    await idb.put('payments', { ...rec, date, status: 'followup', followUpAt: `${date}T${time}:00`, updatedAt: now });
      ...
      ${list.length ? list.map((p) => `
        <div class="rec-card" style="border-right-color:var(--ink-faint);" data-id="${p.id}">
          <div class="rec-card__body">
            ...
            <div class="rec-card__id">${fmtId('P', p.id)} · ${formatJalaliDisplay(p.date)}</div>
            <div class="rec-card__meta">
              <span>${ic('wallet')}${fmtPrice(p.amount)}</span>
              ${p.phone ? `<a class="tel-link" href="tel:${esc(p.phone)}" onclick="event.stopPropagation()">${ic('phone')}${esc(p.phone)}</a>` : ''}
            </div>
