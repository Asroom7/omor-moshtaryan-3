...
async function router() {
  ...
  const topActive = route.name === 'customer-detail' ? 'customers' : route.name === 'product-detail' ? 'products' : route.name === 'payments-history' ? 'dashboard' : route.name;
  renderNav(topActive);
  ...
  function renderPriceRows() {
    ...
    el.innerHTML = priceRows.map((pr, i) => `
      <div class="price-row" data-idx="${i}">
        ...
        <input type="number" class="amt" min="0" placeholder="مبلغ (تومان)" value="${pr.amount}">
    ...
    el.querySelectorAll('.price-row').forEach((rowEl) => {
      ...
      rowEl.querySelector('.amt').addEventListener('input', (e) => { priceRows[idx].amount = e.target.value; });
    });
...
function initComboField(wrap, fieldKey, items, selectedId, opts = {}) {
  ...
  function renderList() {
    ...
    listEl.innerHTML = filtered.length
      ...
      : `<div class="combo-empty">موردی یافت نشد</div>`;
...
function openSaleForm(id, presets = {}) {
  ...
  function renderPriceSuggest(product) {
    ...
    priceSuggestEl.querySelectorAll('button').forEach((btn) => {
      btn.addEventListener('click', () => { priceInput.value = btn.dataset.amt; });
    });
  }
...
function paymentsTodayPanelHTML() {
  const today = todayISO();
  const list = state.payments.filter((p) => p.date === today);
...
/* --- ثبت دستیِ واریزی جدید (فعلاً بدون اتصال پیامک؛ فقط مبلغ و تاریخ) --- */
function openPaymentAddForm() {
  ...
  const html = `
    ...
    <form id="paymentAddForm">
      <div class="field"><label>مبلغ (تومان) *</label><input name="amount" type="number" min="1" required placeholder="مثلاً ۱۰۰۰۰۰۰"></div>
      ${dateFieldHTML('date', formData.date, 'تاریخ واریزی', true)}
      <div class="field__hint" style="margin:-4px 2px 4px;">اطلاعات مشتری (نام، شماره تماس، آدرس) را می‌توانید در مرحله‌ی «تکمیل و ثبت» وارد کنید.</div>
      <div class="modal__actions">
        <button type="button" class="btn secondary block" data-close-modal>انصراف</button>
  ...
  wrap.querySelector('#paymentAddForm').addEventListener('submit', async (e) => {
    ...
    await loadAll(); closeModal(); toast('واریزی ثبت شد'); router();
  });
}

/* --- تکمیل/ویرایش واریزی: اتصال یا ایجاد مشتری و تغییر وضعیت به ثبت‌شده --- */
function openPaymentCompleteForm(id) {
  ...
  const html = `
    ...
    <form id="paymentCompleteForm">
      <div class="field"><label>مبلغ (تومان) *</label><input name="amount" type="number" min="1" required value="${rec.amount}"></div>
      ${dateFieldHTML('date', formData.date, 'تاریخ واریزی', true)}
      <div class="field-row">
      ...
      <div class="modal__actions">
        <button type="button" class="btn secondary block" data-close-modal>انصراف</button>
        <button type="submit" class="btn primary block">${ic('check')}${isDone ? 'ذخیره تغییرات' : 'تأیید و ثبت مشتری'}</button>
      </div>
    </form>`;
  const wrap = openModal(html);
  initDateFields(wrap, formData);
  wrap.querySelector('#paymentCompleteForm').addEventListener('submit', async (e) => {
    ...
    await idb.put('payments', payload);
    await loadAll(); closeModal();
    toast(isDone ? 'واریزی ویرایش شد' : 'واریزی ثبت شد');
    router();
  });
}
